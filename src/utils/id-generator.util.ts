...
export function generateId(type: number, time: number, shard: number, sequenceId: number): string {
    const typePart = type;
...
export function decodeId(input: string): number[] {
    ...
    const time = Number((bigIntValue >> BigInt(19)) & BigInt(0x1ffffffffff)) + INIT_DATE;
...
function isValidId(type: number, time: number, shard: number, sequenceId: number): boolean {
    if (type < 16 && time > INIT_DATE && shard < 512 && sequenceId < 1024) return true;
    return false;
}
