...
async function bootstrap() {
    console.log('Application starting.', process.env.LOG_LEVEL, process.env.NODE_ENV);
    ...
    const app = await NestFactory.create<NestFastifyApplication>(AppModule, new FastifyAdapter(), opts);
    ...
    if (!isEnv(Env.Production)) {
        ...
        const config = new DocumentBuilder()
            .setTitle('Motel API')
            .setDescription('Tìm nhà phòng nhanh chóng')
            .setVersion('1.0')
            // .addBearerAuth()
